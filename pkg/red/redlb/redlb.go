...
//
// 该方法会：
// 1. 读取 boot 信息和 grpc/http 监听地址。
// 2. 把实例信息写入 Redis 并设置过期时间（Ttl）。
// 3. 按心跳间隔持续续期。
// 4. Stop 时主动删除 key。
func (r *Registry) Register(ctx context.Context, serviceName string, opts ...RegisterOption) (*Lease, Endpoint, error) {
	serviceName = r.normalizeServiceName(serviceName)
	if serviceName == "" {
		return nil, Endpoint{}, ErrServiceNameRequired
	}

	ep, key, err := r.buildEndpoint(serviceName, opts...)
	if err != nil {
...
func (r *Registry) buildEndpoint(serviceName string, regOpts ...RegisterOption) (Endpoint, string, error) {
	ro := registerOptions{}
	for _, opt := range regOpts {
		opt(&ro)
	}

	info := r.opt.bootInfoReader()
	ip := firstNonEmpty(ro.ip, info.IntranetIP)
	if ip == "" {
		return Endpoint{}, "", ErrIpRequired
	}

	hostname := firstNonEmpty(ro.hostname, info.Hostname)
	grpcAddr := firstNonEmpty(ro.grpcAddr, r.opt.addrReader("grpc.addr"))
	if strings.TrimSpace(grpcAddr) == "" {
		return Endpoint{}, "", ErrGrpcAddrRequired
	}
	httpAddr := firstNonEmpty(ro.httpAddr, r.opt.addrReader("http.addr"))
	...
	return ep, r.serviceKey(serviceName, instanceId), nil
}
...
// normalizeServiceName 归一化服务名，空值时回退到 boot.Read().Name。
func (r *Registry) normalizeServiceName(serviceName string) string {
	serviceName = strings.TrimSpace(serviceName)
	if serviceName != "" {
		return serviceName
	}
	return strings.TrimSpace(r.opt.bootInfoReader().Name)
}
